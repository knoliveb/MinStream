...
| `lyrics.js` | Módulo de letra sincronizada (`Lyrics`): busca a letra da faixa no LRCLIB (serviço aberto/gratuito), sincroniza linha a linha com a reprodução e preenche os containers de letra dos dois players expandidos, além das duas telas cheias de letra — o overlay do desktop e a página do mobile, que compartilham o mesmo mecanismo de destaque e rolagem. Autocontido — apenas lê o estado do app; carregado **depois** de `app.js`. |
...
- **Fundo que reage à capa (desktop e mobile)**: nos dois players expandidos o fundo é a capa da faixa desfocada sob um gradiente escuro. No desktop ele ainda ganha um tom tirado da **cor dominante da própria capa** (a thumbnail é amostrada num canvas; matiz dominante ponderado por saturação × brilho, com transição suave na troca de faixa e cache por capa). Capas acinzentadas ou ilegíveis por CORS caem num fundo neutro escuro. As camadas ficam no `#exp-backdrop`, irmão do `#expanded-player`, para não rolarem junto com o conteúdo.
...
- **Letra sincronizada (estilo Spotify)**: nos dois players expandidos (desktop e mobile), um container de **Letra** aparece logo após os controles do player e antes de "Videoclipes relacionados". A letra vem do **[LRCLIB](https://lrclib.net)** — serviço open source, gratuito e sem API key (formato LRC):
  - **Cabeçalho fixo estilo Spotify**: o container tem um cabeçalho com o rótulo **"Letra"** e botões circulares (traduzir, compartilhar e — no desktop — expandir em tela cheia); o cabeçalho projeta uma sombra **na própria cor de fundo do container** e o texto **desaparece por baixo dele** conforme rola. O crédito **"Letras fornecidas por LRCLIB"** aparece dentro do container, ao fim da letra, com um bom espaçamento.
  ...
  - **Botões do cabeçalho**: **traduzir** (converte a letra para o idioma do app e volta ao original), **compartilhar** (envia/copia a letra e o link da faixa), **sincronizar** (alterna entre acompanhar a música e o texto completo — preferência persistida) e **expandir** (abre a letra em tela cheia, no desktop). Aparecem quando há letra.
  - **Letra em tela cheia (mobile)**: tocar em qualquer ponto do container de Letra abre uma página dedicada, que **desliza de baixo para cima** e volta pelo caminho inverso ao fechar, revelando o player por trás. Traz o botão de fechar, título e artista, a letra em fonte grande com a linha ativa acompanhando a música (tocar numa linha busca aquele instante), os botões de **traduzir** e **compartilhar**, a barra de progresso com arraste e o **play/pause**. O fundo é a mesma cor tirada da capa. **Só abre quando há letra carregada** — com "Buscando letra…", "Sem letra disponível" ou faixa instrumental o toque não faz nada. Fecha pelo botão, por `Esc`, ao fechar o player, ao desligar a letra nas Configurações ou quando a faixa seguinte não tem letra. Nessa tela, tocar numa linha do container pequeno abre a página em vez de buscar o instante (a busca por toque fica dentro dela).
...
- **Letra em tela cheia**: o container de Letra abre uma página própria ao toque (ver "Letra sincronizada", acima), por cima do player e com transição deslizante nos dois sentidos.
...
| `minstream_exp_layout` | Layout do player expandido no desktop: 'classic' (padrão) ou 'modern' (vídeo e letra lado a lado) |
| `minstream_follows` | Artistas seguidos (nome, foto do perfil e data) exibidos na seção "Seguindo" do Início |
